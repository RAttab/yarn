...
START_TEST(t_bits_basic) {
  ...
  for (yarn_word_t i = 0; i < 3; ++i) {
    for (yarn_word_t j = 0; j < YARN_WORD_BIT_SIZE; j++) {
      ...
      {
	yarn_word_t index = YARN_BIT_INDEX(n);
	fail_if (index != j, 
      ...
      {
	yarn_word_t mask = YARN_BIT_MASK(n);
	fail_if(mask != b, 
      ...
      {
	yarn_word_t null_set = YARN_BIT_SET(0, n);
	fail_if(null_set != b, 
      ...
      {
	yarn_word_t full_set = YARN_BIT_SET(-1, n);
...
START_TEST(t_bits_range_mask) {
  {
    for (yarn_word_t i = 0; i <= YARN_WORD_BIT_SIZE; ++i) {
      yarn_word_t mask = yarn_bit_mask_range(i,i);
